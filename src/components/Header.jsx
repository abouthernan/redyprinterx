  ...
  return (
    ...
    >
      <div class="max-w-7xl flex h-[72px] justify-between p-2 md:py-2 md:px-0 items-center mx-auto">
        <a href="/" className="h-14 w-14 transition-transform duration-300 flex-grow-0">
          <img
        ...
        <nav
          className={`flex bg-blanco rounded-t-3xl md:bg-transparent fixed md:static top-[72px] left-0 right-0 h-screen md:h-auto transition-all translate-x-full md:translate-x-0 duration-500 opacity-0 md:opacity-100 ${menuIsOpen ? 'translate-x-0 opacity-100' : ''
            }`}
        >
          <ul className="flex flex-col md:flex-row gap-4 w-full items-center mt-14 md:mt-0">
            ...
            <li>
              <button id="mega-menu-dropdown-button" data-dropdown-toggle="mega-menu-dropdown" class="flex items-center justify-between w-full py-2 pl-3 pr-4 font-medium hover:text-cielo/80 transition-colors duration-300">
                Equipos <svg class="w-2.5 h-2.5 ml-2.5" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 10 6">
                  <path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="m1 1 4 4 4-4" />
                </svg>
              </button>
              <div id="mega-menu-dropdown" class="absolute w-auto min-w-max z-10 hidden bg-white border border-blanco rounded-lg shadow-md  md:grid-cols-3 ">
                <div class="p-4">
        ...
        <button
          className={`md:hidden w-8 h-8 tex-black flex items-center justify-center border-none outline-none appearance-none ${menuIsOpen ? 'open' : ''
            }`}
